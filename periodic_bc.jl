...
#"""
#function periodic_net_w_lengths(point_set::Array{Array{Float64,1},1}, β)
#    
#    N = length(point_set)
#
...
function periodic_net_w_lengths(point_set::Array{Array{Float64,1},1}, β)
    ...
    for i in 1:8
        this_tile = [point_set[j] + translation_vects[i,:] for j in 1:N]
        mega_set = vcat(mega_set, this_tile)
    end
...
function periodic_net_w_lengths(point_set::Array{Float64,2}, β)
    num_points = size(point_set)[1]
    new_point_set = [point_set[i,:] for i in 1:num_points] #array of arrays
    periodic_net_w_lengths(new_point_set, β)
end
...
function torus_od₂(rn::RoadNetwork)
    g = rn.g
    n = num_nodes(g)
    dists_from_o =  100*ones(n)
    dists_from_center = 100*ones(n)
    for i in 1:n
      pos = g.nodes[i].pos
      
      dists_from_o[i] = norm(pos - [0.5, 0.0])
      dists_from_center[i] = norm(pos - [1.0,0.5])
    end
    o, d = indmin(dists_from_o), indmin(dists_from_center)
